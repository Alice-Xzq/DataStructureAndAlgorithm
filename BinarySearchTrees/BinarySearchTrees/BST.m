...
-(TreeNode *)deleteKey : (int) key fromNode : (TreeNode *) node{
    ...
    if(node.key > key){
        ...
        //when the current node has both child, we need to find and delete the next smallest element and replace the current node with it
        TreeNode *temp = node;
